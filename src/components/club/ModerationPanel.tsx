...
type Props = {
    ...
    onDemote: (id: string | number) => void;
    onRemove: (id: string | number) => void;
    onApprove?: (id: string | number) => void;  // ⬅️ new
    pendingId?: string | number | null;         // ⬅️ new
};

// show Approve when the viewer has power and target is not approved
const canApprove = (me: Role, approved?: boolean) =>
    (me === "owner" || me === "moderator") && approved === false;


const ModerationPanel: React.FC<Props> = ({ me, members, onClose, onPromote, onDemote, onRemove, onApprove, pendingId }) => {
                        ...
                        <li key={m.id} className="py-3">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    {/* ⬇️ add this line to show pending status */}
                                    {m.is_approved === false && <div className="text-xs text-amber-600">Pending approval</div>}
                                </div>

                                <div className="flex gap-2">
                                    {canApprove(me, m.is_approved) && (
                                        <button className="btn" disabled={pendingId===m.id} onClick={() => onApprove?.(m.id)}>Approve</button>
                                    )}
